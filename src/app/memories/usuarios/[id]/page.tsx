  ...
  useEffect(() => {
    const fetchMemories = async () => {
      try {
        ...
        const fetchedMemories: Memory[] = response.data
  ...
  return (
    <div className="mt-8">
      ...
      >
        ...
        voltar para a sua timeline
      </Link>
      <ul className="flex flex-col gap-10 overflow-x-hidden p-8">
        {memories
          .filter((memory) => memory.isPublic) // Filter out non-public memories
          .map((memory) => (
            <li key={memory.id}>
